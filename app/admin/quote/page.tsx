            ...
            <CardContent>
              <div className="overflow-x-auto rounded-xl">
                <table className="w-full text-sm border">
                  ...
                  <tbody>
                    {quotes.map(q => {
                      let pcbSpec: Record<string, unknown> | null = q.pcb_spec as Record<string, unknown> | null;
                      if (typeof q.pcb_spec === 'string') {
                        try { pcbSpec = JSON.parse(q.pcb_spec); } catch { pcbSpec = null; }
                      }
                      return (
                        <tr key={q.id} className="border-b hover:bg-slate-50">
                          <td className="p-2 border">{q.id}</td>
                          <td className="p-2 border">{q.user_id}</td>
                          <td className="p-2 border">{pcbSpec?.pcbType as string ?? '-'}</td>
                          <td className="p-2 border">{pcbSpec?.layers as string ?? '-'}</td>
                          <td className="p-2 border">{q.status || "pending"}</td>
                          <td className="p-2 border">
                            <Button size="sm" variant="outline" onClick={() => router.push(`/admin/quote/orders/${q.id}`)}>Review</Button>
                          </td>
                        </tr>
                      );
                    })}
